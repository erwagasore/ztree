...
- **Type-erased re-entrant walker.** `Walker`/`walker()` let `.skip_children` handlers walk subtrees back through the same rendering pipeline. The function pointer boundary breaks Zig's error-set inference cycle that otherwise forces awkward annotations on renderer methods. `TypedWalker(Error)`/`typedWalker(Error, renderer)` preserve a caller-selected error set for renderers that expose a narrow public error contract. Simple renderers ignore it; complex ones (e.g. Markdown list rendering) store a walker field and call `self.walker.walk(child)`.
